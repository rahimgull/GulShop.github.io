...
switch($_GET["action"]) {
	case "add":
		if(!empty($_POST["qty"])) {
			$productById = $db_handle->runQuery("SELECT * FROM products WHERE product_id='" . $_GET["product_id"] . "'");
			$itemArray = array($productById[0]["product_id"]=>array('product_name'=>$productById[0]["product_name"], 'product_keywords'=>$productById[0]["product_keywords"], 'qty'=>$_POST["qty"], 'product_price'=>$productById[0]["product_price"], 'product_image'=>$productById[0]["product_image"]));
			
			if(!empty($_SESSION["cart_item"])) {
				if(in_array($productById[0]["product_id"],array_keys($_SESSION["cart_item"]))) {
					foreach($_SESSION["cart_item"] as $k => $v) {
							if($productById[0]["product_keywords"] == $k) {
...
<!-- Bootstrap style responsive -->.	<link href="themes/css/bootstrap-responsive.min.css" rel="stylesheet"/>
...
<form action="" method="post" enctype="multipart/form-data">
    ...
    <table align="center" width="600px" bgcolor="skyblue" border="1">
        <tr>
            <td colspan="6"><h2>Update Your cart or checkout</h2></td>
        </tr>
      <br>
        <tr align="center">
            <th>Remove</th>
...
<?php		
    ...
    while ($p_price=mysqli_fetch_array($run_price)) {
     ...
     while ($pp_price = mysqli_fetch_array($run_pro_price)) {
       ...
       $unit_price = $pp_price['product_price'];
       $total_price = array_sum($product_price);
        ...
        <tr>
            ...
            <td>
            <img src="admin_area/product_images/<?php echo $product_image;?>" width="60" height="60" class="cart-item-image"/><?php echo $product_name; ?></td>
		...
		?>
            ...
            <td  style="text-align:right;"><?php echo "$ ". number_format($item_price,2); ?></td>
            	...
            	<td style="text-align:center;"><a href="cart3.php?action=remove&product_id=<?php echo $item["product_id"]; ?>" class="btnRemoveAction"><img src="icon-delete.png" alt="Remove Item" /></a></td>
        </tr>
